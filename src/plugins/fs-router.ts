...
function template(routes: ScanedItem[]) {
  ...
  return (
    <LocationProvider>
      <Router>
        ${routes
          .map(route =>
             `<Route path="${route.routePath}" component={${route.componentName}} />`,
          )
          .join('\n        ')}
      </Router>
    </LocationProvider>
...
async function scanRoutes(dirPath: string, projectPath: string = '~/pages'): Promise<ScanedItem[]> {
